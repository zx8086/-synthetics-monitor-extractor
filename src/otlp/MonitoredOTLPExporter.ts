...
import { log, warn, err, debug } from "../utils/logger.js";
import { otlpConfig } from "./otlpConfig.js";
...
export abstract class MonitoredOTLPExporter<T> {
  ...
  constructor(
    ...
    if (
      ...
      this.logIntervalMs = config.openTelemetry.metricIntervalMs || 300000;
    } else {
  ...
  protected logSystemResources(): void {
    ...
    `);
    debug(`System CPU Usage (1m average): ${cpuUsage?.toFixed(2) || 'N/A'}`);
